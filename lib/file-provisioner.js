var path = require('path'),
  Promise = require('bluebird'),
  promisify = require('promisify-node'),
  fs = promisify('fs-extra'),
  FileList = require('./filelist'),
  spawn = require('child_process').spawn,
  _ = require('underscore'),
  util = require('util');

var FileProvisioner = module.exports = function(config) {
  this.fileList = new FileList();
  this.dryRun = config.dryRun;
}

FileProvisioner.prototype.provisionFiles = function(config) {
  var rootPaths = config.workBook.mapCellsToJSON({
      col: config.col,
      rowStart: config.rowStart,
      rowEnd: config.rowEnd,
      keyMap: config.keyMap
    }),
    that = this;

  if (!rootPaths.e57nRootPath) {
    return;
  }

  var e57nSrcPath = rootPaths.e57nRootPath.replace('/data', '/tmp/duraark-data/data'), // FIXXME!
    e57nGlob = '*.e57n',
    destPath = rootPaths.destinationRootPath.replace('~/duraark-sessions', '/afs/cgv.tugraz.at/Fraunhofer/Projekte/DuraArK/datasets/sessions'), // FIXXME!
    e57nDestPath = path.join(destPath, 'master'),
    ifcSrcPath = rootPaths.ifcRootPath.replace('/data', '/tmp/duraark-data/data'),
    ifcGlob = '*.ifc',
    ifcDestPath = path.join(destPath, 'derivative_copy');

  if (!e57nSrcPath || e57nSrcPath === '' || e57nSrcPath === '-') {
    return;
  }

  return this.createFolderStructure(destPath)
    .then(that.copyFiles(e57nSrcPath, e57nGlob, e57nDestPath))
    .then(that.copyFiles(ifcSrcPath, ifcGlob, ifcDestPath))
    .then(function() {

      return {
        srcPath: rootPaths.e57nRootPath.replace('/data', '/tmp/duraark-data/data'),
        srcGlob: '*.e57n',
        destPath: rootPaths.destinationRootPath
      };
    }).catch(function(err) {
      console.log('err: ' + JSON.stringify(err, null, 4));
    });
}

FileProvisioner.prototype.copyFiles = function(srcPath, srcGlob, destPath) {
  var that = this;

  console.log('[FileProvisioner] copying %s to %s', path.join(srcPath, srcGlob), destPath);

  return this.fileList.getFiles(srcPath, srcGlob).then(function(files) {
    _.forEach(files, function(file) {
      if (that.dryRun) {
        console.log('[FileProvisioner] FINISHED DRY RUN copying %s to %s', file, destPath);
        return true;
      } else {
        return fs.copy(file, path.join(destPath, path.basename(file))).then(function() {
          console.log('[FileProvisioner] FINISHED copying %s to %s', file, destPath);
          return true;
        }).catch(function(err) {
          console.log('copy error: ' + err);
        });
      }
    });
  });
}

FileProvisioner.prototype.createFolderStructure = function(rootPath) {
  // console.log('creating folder structure for: ' + rootPath);

  return new Promise(function(resolve, reject) { // FIXXME: make async!
    fs.mkdirpSync(rootPath);
    fs.mkdirpSync(path.join(rootPath, 'master'));
    fs.mkdirpSync(path.join(rootPath, 'derivative_copy'));
    fs.mkdirpSync(path.join(rootPath, 'tmp'));
  });
}
